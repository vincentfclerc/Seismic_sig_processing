...
for file in range(1, 2): #rajouter un zero dans le nom du fichier si <10
    ...
    sig_win_X = np.zeros(shape=(58*2000, (LENGTH_SIGNAL*3)+2))
    sig_win_Y = np.zeros(shape=(58*2000, (LENGTH_SIGNAL*3)+2))
    sig_win_Z = np.zeros(shape=(58*2000, (LENGTH_SIGNAL*3)+2))
    c_it = 0
    c_it_empty = 0
    for trace in range(1000):
        ...
        for startf in array_starts_windows:
            start=int(startf)
            sig_win[c_it][2:(LENGTH_SIGNAL+2)] = matrix_dataset[trace][start+4:(start+4+LENGTH_SIGNAL)]
            sig_win[c_it][(LENGTH_SIGNAL+2):(2*LENGTH_SIGNAL+2)] = matrix_dataset[trace][start+6006:(start+LENGTH_SIGNAL+6006)]
            sig_win[c_it][(2*LENGTH_SIGNAL+2):(3*LENGTH_SIGNAL+2)] = matrix_dataset[trace][start+12008:(start+LENGTH_SIGNAL+12008)]
    
            sig_win[c_it][0] = pick_P-start
            sig_win[c_it][1] = pick_S-start
            c_it += 1
            
            
    
    #np.savetxt(f'C:/Users/vince/Dropbox/tomos_proto_classif/seismic_traces/file_{file}_all_windows.txt', sig_win, fmt='%1.1f')
